<template>
    <el-table height="70vh" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
        :data="judgeList" :header-cell-style="{ background: '#f2f6fd', color: '#000' }">
        <el-table-column prop="id" label="序号" width="80" align="center"> </el-table-column>
        <el-table-column label="头像" width="150" align="center">
            <template slot-scope="scope">
                <el-image :src="scope.row.base64">
                </el-image>
            </template>
        </el-table-column>
        <el-table-column label="职位" width="250" align="center">
            <template slot-scope="scope">
                <div style="display: flex;align-items: center;justify-content: center;">
                    <p class="my-task-auth margin-icon" v-if="scope.row.isadmin">
                        <i class="el-icon-s-custom"></i>裁判管理员
                    </p>
                    <p class="my-task-success margin-icon" v-else>
                        <i class="el-icon-user-solid"></i>常驻裁判
                    </p>
                </div>
            </template>
        </el-table-column>
        <el-table-column prop="chinaname" label="姓名" width="250">
        </el-table-column>
        <el-table-column label="qq号" width="250">
            <template #default="{ row }">
                {{ row.qqnumber || '未绑定qq' }}
            </template>
        </el-table-column>
        <el-table-column label="邮箱" width="auto">
            <template #default="{ row }">
                {{ row.email || '-' }}
            </template>
        </el-table-column>
        <el-table-column label="操作" width="150" align="center">
            <template #default="{ row }">
                <el-button style="color:#f40" type="text" @click="refuseUser(row)">辞退裁判</el-button>
            </template>
        </el-table-column>
    </el-table>
</template>

<script>
import { getUserRoles } from "@/api/schedule/index";
import { refuseCom } from "@/api/admin/index.js";
export default {
    name: 'judgetManage',
    data() {
        return {
            judgeList: [],
            loading: false,
        };
    },
    methods: {
        async initData() {
            try {
                this.loading = true;
                const { data, status } = await getUserRoles({ opname: "Judge" });
                if (status !== 200) throw new Error('服务端异常，请联系网站管理员！');
                this.judgeList = data;
            } catch (error) {
                this.$message.error(error.message);
            } finally {
                this.loading = false;
            }
        },
        refuseUser(row) {
            this.$confirm(`您确定将解说${row.chinaname}劝退吗？, 一旦操作无法恢复，是否继续?`, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                refuseCom(row.id).then(() => {
                    this.$message.success("操作成功！");
                    this.initData();
                })
                    .catch((err) => {
                        this.$message.error(err.message);
                    })
            })
        }
    },
    created() {
        this.initData();
    },
}
</script>
<style lang='less' scoped></style>